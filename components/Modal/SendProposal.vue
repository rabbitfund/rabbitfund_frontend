...
<template>
  <div
    ...
    tabindex="-1"
    aria-hidden="true"
    class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full overflow-y-auto overflow-x-hidden md:inset-0"
  >
    <div
      class="relative max-h-full w-full lg:w-[calc(theme('screens.lg')*2/3)] xl:w-[calc(theme('screens.xl')*2/3)]"
    >
      <button
        @click="closeModal()"
        type="button"
        class="absolute right-5 top-5 rounded bg-white hover:bg-primary-light"
      >
        <svg
          width="24"
          height="24"
